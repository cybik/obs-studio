...
static bool create_video_stream(struct ffmpeg_mux *ffm)
{
...
static bool create_audio_stream(struct ffmpeg_mux *ffm, int idx)
{
	AVCodecContext *context;
	AVStream *stream;
	void *extradata = NULL;
...
static bool init_streams(struct ffmpeg_mux *ffm)
{
	...
	if (ffm->params.tracks) {
		...
		for (int i = 0; i < ffm->params.tracks; i++) {
			bool success = create_audio_stream(ffm, i);
			if (!success) {
...
static inline int get_index(struct ffmpeg_mux *ffm,
		struct ffm_packet_info *info)
{
	return info->type == FFM_PACKET_VIDEO ?
		0 : info->index + ffm->params.has_video;
}
